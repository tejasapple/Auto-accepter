...
# ==========================================
# ⚙️ CONFIGURATION (यहाँ अपनी डिटेल्स डालें)
# ==========================================
API_ID = 31660355  # अपना API ID यहाँ डालें (my.telegram.org से)
API_HASH = "78292fcf0b3c508b3257e9dda9728df4" # अपना API HASH यहाँ डालें
BOT_TOKEN = "8972078260:AAENtp-9JaIo5ykLuEs9B1er8l6T7WvuEQo" # @BotFather से मिला टोकन
MONGO_DB_URI = "mongodb+srv://Tejas7xx:mrxtejas7@cluster0.akhlgjf.mongodb.net/?appName=Cluster0" # अपनी MongoDB URL डालें
...
async def start_command(client, message):
    ...
    # Permissions: All standard admin rights EXCEPT Anonymous Admin
    # invite_users, manage_chat, restrict_members, promote_members, change_info, post_messages, edit_messages, delete_messages
    ...
    # Save User to DB
    await users_col.update_one({"user_id": message.from_user.id}, {"$set": {"user_id": message.from_user.id}}, upsert=True)
...
async def auto_accept_requests(client, request: ChatJoinRequest):
    ...
    try:
        ...
        await users_col.update_one({"user_id": user.id}, {"$set": {"user_id": user.id}}, upsert=True)
        await chats_col.update_one({"chat_id": chat.id}, {"$set": {"chat_id": chat.id, "title": chat.title}}, upsert=True)
        
        # 3. Send Welcome DM
        welcome_text = f"**Welcome to {chat.title}, {user.first_name}! 🎉**\n\nYour request has been approved."
...
async def broadcast_to_users(client, message):
    ...
    for user in users:
        try:
            ...
            # If user blocked the bot, remove them from DB to keep DB clean
