...
# ==========================================
API_ID = 31660355  # अपना API ID यहाँ डालें
...
ADMIN_ID = 8884734704  # अपना खुद का टेलीग्राम User ID डालें
...
async def start_command(client, message):
    ...
    await users_col.update_one(
        ...
        {"$set": {"user_id": message.from_user.id, "name": message.from_user.first_name}}, 
        upsert=True
    )
...
# ==========================================
# ✅ AUTO ACCEPT & WELCOME DM LOGIC (FIXED)
# ==========================================
...
async def auto_accept_requests(client, request: ChatJoinRequest):
    ...
    # 1. सबसे पहले DM भेजें (ताकि Telegram की Temporary Permission यूज़ हो सके)
    try:
    ...
    # 2. DM भेजने के बाद रिक्वेस्ट अप्रूव करें
    try:
        ...
        print(f"Error approving request for {user.id} in {chat.id}: {e}")
        
    # 3. फ्यूचर ब्रॉडकास्ट के लिए यूजर और चैट को डेटाबेस में सेव करें
    try:
        await users_col.update_one(
            {"user_id": user.id}, 
